 ...
 * for more informaiton about the model.
 *
 ...
 *
 * In this example, the input image is stored in a csv file, which contains in a
 * row both the digit lable (a number from 0 to 9) and the 784 pixel values (a
 * number from 0 to 255). Please use the example_digit.csv file for reference.
 * Such a csv digit file can be created by copying any row other than the first
 * from file mnist_test.csv at https://www.kaggle.com/oddrationale/mnist-in-csv.
 */
