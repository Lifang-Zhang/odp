...
static int fill_model_param(const char *model_name, odp_ml_model_param_t *model_param)
{
	...
	if ((global.ml_config.load_mode_mask & ODP_ML_COMPL_MODE_POLL) ||
		...
		model_param->max_compl_id = MAX_COMPL_ID;
...
static int check_load_poll_sync(void)
{
	if (global.disabled)
		return ODP_TEST_INACTIVE;
...
static int check_load_event_sync(void)
{
	if (global.disabled)
		return ODP_TEST_INACTIVE;
...
static void test_ml_debug(void)
{
	...
	CU_ASSERT_NOT_EQUAL(u64, odp_ml_model_to_u64(ODP_ML_MODEL_INVALID));
...
static void test_ml_model_create(void)
{
	...
	CU_ASSERT_EQUAL(i, max_models);
	max_models = i;

	/* Destroy valid models */
	for (i = 0; i < max_models; i++)
		CU_ASSERT_EQUAL_FATAL(odp_ml_model_destroy(models[i]), 0);
}
...
static void test_ml_model_lookup(void)
{
	...
	CU_ASSERT_NOT_EQUAL_FATAL(model_lookup, ODP_ML_MODEL_INVALID);
	CU_ASSERT_EQUAL(odp_ml_model_to_u64(global.ml_model), odp_ml_model_to_u64(model_lookup));

	/* Look up model with a different name, should return invalid handle */
	model_lookup = odp_ml_model_lookup("diff");
	CU_ASSERT_EQUAL_FATAL(model_lookup, ODP_ML_MODEL_INVALID);

	model2 = odp_ml_model_create(MODEL_NAME, &global.model_param);
	CU_ASSERT_NOT_EQUAL_FATAL(model2, ODP_ML_MODEL_INVALID);
	CU_ASSERT_NOT_EQUAL(odp_ml_model_to_u64(global.ml_model), odp_ml_model_to_u64(model2));
	...
	CU_ASSERT_EQUAL(odp_ml_model_destroy(model2), 0);
}
...
static void test_ml_model_info(void)
{
	...
	CU_ASSERT_STRING_EQUAL(ml_info.name, MODEL_NAME);
	CU_ASSERT_EQUAL(ml_info.model_version, 1);
	CU_ASSERT_EQUAL(ml_info.num_inputs, NUM_INPUTS);
	CU_ASSERT_EQUAL(ml_info.num_outputs, NUM_OUTPUTS);

	num_ret = odp_ml_model_input_info(global.ml_model, input_info, NUM_INPUTS);
	CU_ASSERT_EQUAL(num_ret, NUM_INPUTS);
	CU_ASSERT_STRING_EQUAL(input_info[0].name, "x");
	CU_ASSERT_EQUAL(input_info[0].shape.num_dim, 1);
	CU_ASSERT_EQUAL(input_info[0].shape.dim[0], 1);
	CU_ASSERT_EQUAL((int)input_info[0].data_type, ODP_ML_DATA_TYPE_INT32);
	...
	input_info[1].data_type = ODP_ML_DATA_TYPE_STR;
	...
	CU_ASSERT_EQUAL(input_info[1].data_type, ODP_ML_DATA_TYPE_STR);

	num_ret = odp_ml_model_output_info(global.ml_model, output_info, NUM_OUTPUTS);
	CU_ASSERT_EQUAL(num_ret, NUM_OUTPUTS);
	CU_ASSERT_STRING_EQUAL(output_info[0].name, "y");
	CU_ASSERT_EQUAL(output_info[0].shape.num_dim, 1);
	CU_ASSERT_EQUAL(output_info[0].shape.dim[0], 1);
	CU_ASSERT_EQUAL((int)output_info[0].data_type, ODP_ML_DATA_TYPE_INT32);
	...
	CU_ASSERT_EQUAL(output_info[1].shape.num_dim, 98876);
}
...
static void test_ml_model_load(void)
{
	...
	CU_ASSERT_NOT_EQUAL_FATAL(test_model, ODP_ML_MODEL_INVALID);

	ret = odp_ml_model_load(test_model, &result);
	CU_ASSERT_EQUAL(ret, 0);
	CU_ASSERT_EQUAL(result.error_code, 0);

	num_load = global.ml_config.max_models_loaded;
	for (i = 0; i < num_load - 1; i++) { /*It has been loaded one time already*/
		ret = odp_ml_model_load(test_model, NULL);
		CU_ASSERT_EQUAL(ret, 0);
	}
	...
	CU_ASSERT_EQUAL(odp_ml_model_destroy(test_model), 0);
}
...
static void test_ml_model_load_async_poll(void)
{
	...
	for (int i = 0; i < TIMEOUT; i++) {
		ret = odp_ml_model_load_status(global.ml_model, compl_id, &result);
		if (ret)
			break;
	...
	CU_ASSERT(ret > 0);
	CU_ASSERT_EQUAL(result.error_code, 0);
	CU_ASSERT_PTR_EQUAL(result.user_ptr, user_ptr);
	/* odp_ml_model_load does not modify data in user_ptr */
	if (result.user_ptr)
		CU_ASSERT_EQUAL(*(int *)result.user_ptr, dummy);

	CU_ASSERT_EQUAL_FATAL(odp_ml_model_unload(global.ml_model, NULL), 0);
}
...
get_result_from_ml_compl_event(odp_ml_load_result_t *load_result, odp_ml_run_result_t *run_result)
{
	...
	CU_ASSERT_NOT_EQUAL(ev, ODP_EVENT_INVALID);
	if (ev == ODP_EVENT_INVALID)
		return -1;

	ev_type = odp_event_type(ev);
	CU_ASSERT_EQUAL(from_queue, global.queue);
	CU_ASSERT_EQUAL(ev_type, ODP_EVENT_ML_COMPL);
	if (from_queue != global.queue || ev_type != ODP_EVENT_ML_COMPL)
		return -1;

	compl = odp_ml_compl_from_event(ev);
	CU_ASSERT_NOT_EQUAL(compl, ODP_ML_COMPL_INVALID);
...
static void test_ml_model_load_async_event(void)
{
	...
	CU_ASSERT_PTR_NOT_EQUAL_FATAL(compl, ODP_ML_COMPL_INVALID);
...
static void test_ml_model_unload(void)
{
	int ret;
	odp_ml_load_result_t result;

	/* Load model in order to test unload */
	ret = odp_ml_model_load(global.ml_model, NULL);
	CU_ASSERT_EQUAL_FATAL(ret, 0);

	/* Test when result is not NULL */
	ret = odp_ml_model_unload(global.ml_model, &result);
	CU_ASSERT_EQUAL(ret, 0);
	CU_ASSERT_EQUAL(result.error_code, 0);
}

/* Test asynchronous model unloading in ODP_ML_COMPL_MODE_POLL mode */
static void test_ml_model_unload_async_poll(void)
{
	int ret;
	odp_ml_compl_param_t compl_param;
	int dummy = 6;
	...
	/* Return when odp_ml_model_unload_start() failed */
	if (ret) {
		ret = odp_ml_model_unload(global.ml_model, NULL);
		CU_ASSERT_EQUAL_FATAL(ret, 0);
		return;
	 ...
	 * status */
	for (int i = 0; i < TIMEOUT; i++) {
		ret = odp_ml_model_unload_status(global.ml_model, compl_id, &result);
		if (ret)
			break;
...
/* Test asynchronous model unloading in ODP_ML_COMPL_MODE_EVENT mode */
static void test_ml_model_unload_async_event(void)
{
	int ret;
	odp_ml_compl_t compl;
	odp_ml_load_result_t result;
	...
	compl = odp_ml_compl_alloc(global.compl_pool);
	CU_ASSERT_PTR_NOT_EQUAL(compl, ODP_ML_COMPL_INVALID);

	if (compl == ODP_ML_COMPL_INVALID) {
		CU_ASSERT_EQUAL(odp_ml_model_unload(global.ml_model, NULL), 0);
		return;
	}
	...
	/* Return when odp_ml_model_unload_start() failed */
	if (ret) {
		ODPH_ERR("odp_ml_model_unload_start() failed\n");
		odp_ml_compl_free(compl);
		ret = odp_ml_model_unload(global.ml_model, NULL);
		CU_ASSERT_EQUAL_FATAL(ret, 0);
		return;
...
static void run_model_batch_add(void)
{
	...
	odp_ml_data_seg_t input_segs[2];
	odp_ml_data_seg_t output_seg;
	...
	CU_ASSERT_NOT_EQUAL(model, ODP_ML_MODEL_INVALID);
	if (!model)
		return;

	if (odp_ml_model_load(model, NULL)) {
		CU_ASSERT_EQUAL(odp_ml_model_destroy(model), 0);
		return;
	}
	...
	data.output_seg = &output_seg;
	output_seg.size = sizeof(y);
	output_seg.addr = y;
	...
	for (int i = 0; i < MAX_BATCH_SIZE; i++) {
		...
		for (uint32_t j = 0; j < batch_size * NUM_COLUMN; j++)
			CU_ASSERT_EQUAL(y[j], y_expected[j]);

		batch_size--;
	}

	/* Result can be NULL, when output batch size is not needed */
	run_param.result = NULL;
	ret = odp_ml_run(model, &data, &run_param);
	CU_ASSERT_EQUAL(ret, 1);

fail:
	CU_ASSERT_EQUAL_FATAL(odp_ml_model_unload(model, NULL), 0);
	CU_ASSERT_EQUAL(odp_ml_model_destroy(model), 0);
}
...
static void run_global_ml_model(void)
{
	...
	CU_ASSERT_EQUAL(ret, 1);
	CU_ASSERT_FALSE(result.error_code);
	CU_ASSERT_EQUAL(*(int32_t *)global.output_seg.addr, global.y_expected);
...
static void test_ml_run_multi(void)
{
	...
	ret = odp_ml_run_multi(global.ml_model, data, param, RUN_NUM);
	CU_ASSERT_EQUAL(ret, RUN_NUM);
	CU_ASSERT_FALSE(result[0].error_code);
	CU_ASSERT_FALSE(result[1].error_code);
	CU_ASSERT_EQUAL(*(int32_t *)global.output_seg.addr, global.y_expected);
	CU_ASSERT_EQUAL(*(int32_t *)output_seg.addr, y_expected);
...
static void test_ml_model_run_async_event(void)
{
	...
	CU_ASSERT_EQUAL(*(int32_t *)outputs[0].addr, global.y_expected);

	/* Test also without result struct output */
	CU_ASSERT_EQUAL(odp_ml_compl_run_result(compl, NULL), 0);
...
static void test_ml_model_run_async_poll(void)
{
	...
	/* Return when odp_ml_run_start() failed */
	if (ret != 1) {
		/* Unload model */
		ret = odp_ml_model_unload(global.ml_model, NULL);
		CU_ASSERT_EQUAL_FATAL(ret, 0);
	...
	for (int i = 0; i < TIMEOUT; i++) {
		ret = odp_ml_run_status(global.ml_model, compl_id, &result);
		if (ret)
			break;
...
static void test_ml_run_start_multi(void)
{
	...
	compl_param[1].compl_id = compl_id;
	/* user_ptr structure maintains the output data pointer for output retrieval */
	compl_param[1].user_ptr = &output_seg;

	memset(global.output_seg.addr, 0, sizeof(int32_t));

	ret = odp_ml_run_start_multi(global.ml_model, data, compl_param, NULL, RUN_NUM);
	CU_ASSERT_EQUAL(ret, RUN_NUM);
	...
	CU_ASSERT_FALSE(run_result[0].error_code);
	CU_ASSERT_PTR_EQUAL(run_result[0].user_ptr, &global.output_seg);
	outputs[0] = (odp_ml_data_seg_t *)run_result[0].user_ptr;
	CU_ASSERT_EQUAL(*(int32_t *)outputs[0][0].addr, global.y_expected);
	...
	for (int i = 0; i < TIMEOUT; i++) {
		ret = odp_ml_run_status(global.ml_model, compl_id, &run_result[1]);
		if (ret)
			break;
	...
	CU_ASSERT(ret > 0);
	CU_ASSERT_FALSE(run_result[1].error_code);
	CU_ASSERT_PTR_EQUAL(run_result[1].user_ptr, &output_seg);
	CU_ASSERT_EQUAL(*(int32_t *)outputs[1][0].addr, y_expected);
...
odp_testinfo_t ml_suite[] = {
	...
	/* Synchronous odp_ml_model_unload is used to unload the loaded model */
	ODP_TEST_INFO_CONDITIONAL(test_ml_model_load_async_poll, check_load_poll_sync),
	ODP_TEST_INFO_CONDITIONAL(test_ml_model_load_async_event, check_load_event_sync),
	/* Synchronous odp_ml_model_load is used to load the model to test unload */
	ODP_TEST_INFO_CONDITIONAL(test_ml_model_unload, check_load_sync),
	ODP_TEST_INFO_CONDITIONAL(test_ml_model_unload_async_poll, check_load_poll_sync),
	ODP_TEST_INFO_CONDITIONAL(test_ml_model_unload_async_event, check_load_event_sync),
...
odp_suiteinfo_t ml_suites[] = {
	{"ML", ml_suite_init, ml_suit_term, ml_suite},
	ODP_SUITE_INFO_NULL
};
