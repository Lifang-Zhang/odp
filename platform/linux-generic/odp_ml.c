...
#define ML_MAX_IO_SEGS 6
#define ML_MAX_COMPL_ID 32
#define ML_MAX_CONFIG_STR_LEN 65
#define ML_MAX_MODEL_SIZE 1073741824 /* 1 Gibibyte */
...
typedef struct ml_model_t {
	...
	uint64_t		output_sizes[CONFIG_ML_MAX_INPUTS];
...
typedef struct ml_global_t {
	...
	OrtEnv			*env; /* Should it be per session */
	OrtMemoryInfo		*memory_info; /* Not sure if it can be global */
...
{
	odp_ml_capability_t *capa = &_odp_ml_glb->capa;
	...
	if (!(capa->load.compl_mode_mask & config->load_mode_mask)) {
		_ODP_ERR("load_mode_mask %u is not supported!\n", config->load_mode_mask);
		return -1;
	}

	if (!(capa->run.compl_mode_mask & config->run_mode_mask)) {
		_ODP_ERR("run_mode_mask %u is not supported!\n", config->run_mode_mask);
		return -1;
	}
...
{
	odp_bool_t first_dyn = false;
	uint32_t dyn_cnt = 0;

	for (uint32_t i = 0; i < shape->num_dim; i++) {
		if (dims[i] == 0) {
			_ODP_ERR("Dimension value: %ld must be at least 1\n", dims[i]);
			return -1;
	...
	} else if (dyn_cnt == 1 && first_dyn && (shape->num_dim > 1)) {
...
static uint64_t get_total_input_size(uint32_t num_inputs, const odp_ml_input_info_t *input_info,
				     const uint64_t *input_sizes)
{
	...
	for (uint32_t i = 0; i < num_inputs; i++) {
		if (input_info[i].shape.type == ODP_ML_SHAPE_STATIC)
			total_size += input_sizes[i];

		if (input_info[i].shape.type == ODP_ML_SHAPE_BATCH) {
			size = input_info[i].data_type_size;

			for (size_t j = 0; j < input_info[i].shape.num_dim; j++) {
				if (input_info[i].shape.dim[j] != ODP_ML_DIM_DYNAMIC)
					size *= input_info[i].shape.dim[j];
				else
					size  *= input_info[i].shape.dim_max[j];
			}
			total_size += size;
		}
	}
...
static int create_model_shm_stash(ml_model_t *mdl)
{
	int i;
	int32_t ret;
	uintptr_t tmp;
	...
	sprintf(shm_name, "_odp_ml_model_%d_shm", mdl->info.index);
	total_input_size = get_total_input_size(mdl->info.num_inputs,
						mdl->input_info, mdl->input_sizes);

	shm = odp_shm_reserve(shm_name, CONFIG_ML_MAX_CONCUR_INFERS * total_input_size,
			      ODP_CACHE_LINE_SIZE, 0);
	if (shm == ODP_SHM_INVALID) {
		_ODP_ERR("Reserving ML model shared mem failed\n");
		return -1;
	}

	shm_addr = odp_shm_addr(shm);
	if (shm_addr == NULL) {
		_ODP_ERR("Alloc shared mem from ML model shm failed\n");

		if (odp_shm_free(shm))
			_ODP_ERR("ML model shm %s free failed\n", shm_name);

		return -1;
	}

	odp_stash_param_init(&stash_param);
	stash_param.num_obj    = CONFIG_ML_MAX_CONCUR_INFERS;
	stash_param.obj_size   = sizeof(uintptr_t);
	stash_param.cache_size = 0;
	stash_param.strict_size = true;

	sprintf(stash_name, "_odp_ml_model_%d_stash", mdl->info.index);
	stash = odp_stash_create(stash_name, &stash_param);

	if (stash == ODP_STASH_INVALID) {
		_ODP_ERR("Creating ML model stash failed\n");
	...
	for (i = 0; i < CONFIG_ML_MAX_CONCUR_INFERS; i++) {
		uintptr_t shm_addr_uintptr = (uintptr_t)shm_addr;

		ret = odp_stash_put_ptr(stash, &shm_addr_uintptr, 1);
		if (ret != 1)
			break;

		shm_addr = shm_addr + total_input_size;
	}

	if (ret != 1) {
		for (int j = 0; j < i; j++) {
			ret = odp_stash_get_ptr(stash, &tmp, 1);
			if (ret != 1) {
				_ODP_ERR("ML model stash get failed: %u\n", i);
				break;
			}
		}

		if (odp_stash_destroy(stash) || odp_shm_free(shm))
			_ODP_ERR("ML model stash destroy or shm free failed\n");

		return -1;
	}

	mdl->model_shm = shm;
	mdl->model_stash = stash;
	return 0;
}
	...
	for (uint32_t i = 0; i < mdl->info.num_inputs; i++) {
		...
		for (uint32_t j = 0; j < shape->num_dim; j++) {
			if (shape->dim[j] == ODP_ML_DIM_DYNAMIC && !shape->dim_max[j]) {
				_ODP_ERR("Missing dim_max[%u] for dynmic sized input[%u], please"
...
odp_ml_model_t odp_ml_model_create(const char *name, const odp_ml_model_param_t *param)
{
	...
	if (check_io_shape(mdl) || create_model_shm_stash(mdl)) {
		mdl->state = ML_STATE_FREE;
		reset_mdl_info_sizes(mdl);
		odp_ticketlock_unlock(&mdl->lock);
...
int odp_ml_model_destroy(odp_ml_model_t model)
{
	...
	if (mdl->state == ML_STATE_LOADED) {
		_ODP_ERR("Model not unloaded\n");
		odp_ticketlock_unlock(&mdl->lock);
		return -1;
	}

	if (odp_shm_free(mdl->model_shm)) {
		_ODP_ERR("ML model shm free failed\n");
		odp_ticketlock_unlock(&mdl->lock);
		return -1;
	}

	if (odp_stash_destroy(mdl->model_stash)) {
		_ODP_ERR("ML model stash destroy failed\n");
		odp_ticketlock_unlock(&mdl->lock);
		return -1;
	}
...
int odp_ml_model_load(odp_ml_model_t model, odp_ml_load_result_t *result)
{
	...
	if (odp_unlikely(!(_odp_ml_glb->ml_config.load_mode_mask & ODP_ML_COMPL_MODE_SYNC))) {
		_ODP_ERR("Synchronous load is not enabled!\n");
		result_local.error_code = ML_FEATURE_NOT_SUPPORTED;
		goto load_fail;
	}
	...
	odp_ticketlock_lock(&mdl->lock);
	if (odp_unlikely(mdl->state == ML_STATE_FREE)) {
		_ODP_ERR("Model has not been created yet or has been destroyed\n");
		odp_ticketlock_unlock(&mdl->lock);
		result_local.error_code = ML_NOT_CREATED;
		goto load_fail;
	}

	if (odp_unlikely(mdl->num_loaded >= _odp_ml_glb->ml_config.max_models_loaded)) {
		_ODP_ERR("Number of model loading exceeds max number of loading concurrently(%u)\n",
			 _odp_ml_glb->ml_config.max_models_loaded);
		odp_ticketlock_unlock(&mdl->lock);
		result_local.error_code = ML_LOADED;
		goto load_fail;
	}
...
int odp_ml_model_unload(odp_ml_model_t model, odp_ml_load_result_t *result)
{
	...
	if (odp_unlikely(!(_odp_ml_glb->ml_config.load_mode_mask & ODP_ML_COMPL_MODE_SYNC))) {
		result_local.error_code = ML_FEATURE_NOT_SUPPORTED;
		_ODP_ERR("Synchronous unload is not enabled!\n");
		goto unload_fail;
	}
...
int odp_ml_model_unload_start(odp_ml_model_t model, const odp_ml_compl_param_t *compl_param)
{
	...
	/* Upon completion of unloading (either succeeded or failed), send a
	 * completion event to the given queue */
	if (compl_param->mode == ODP_ML_COMPL_MODE_EVENT) {
		odp_ml_load_result_t *result;
		odp_buffer_t buf = (odp_buffer_t)(uintptr_t)compl_param->event;
...
/* This function assumes that parameters to odp_ml_model_run() are valid */
static inline void fill_input_sizes(const ml_model_t *mdl_info, ml_input_t *inputs,
				    const odp_ml_run_param_t *param)
{
	uint64_t size;

	for (uint32_t i = 0; i < mdl_info->info.num_inputs; i++) {
		size = mdl_info->input_sizes[i];

		/* Input with dynamic batch size */
		if (!size) {
			size = get_num_elem(param->batch_size, &mdl_info->input_info[i].shape);
			size *= mdl_info->input_info[i].data_type_size;
		}
...
/* This function assumes that parameters to odp_ml_model_run() are valid */
static int fill_input_addrs(const odp_ml_data_t *data, ml_input_t *inputs,
			    uint32_t num_model_inputs, ml_model_t *mdl)
{
	uintptr_t buf_addr;
	uint32_t input_index = 0;
	uint64_t size_combined = 0;
	odp_bool_t index_new = true;

	/* No segmentation */
	if (data->num_input_seg == num_model_inputs) {
		for (uint32_t i = 0; i < num_model_inputs; i++)
			inputs[i].addr = data->input_seg[i].addr;

		return 0;
	}

	if (odp_stash_get_ptr(mdl->model_stash, &buf_addr, 1) != 1) {
		_ODP_ERR("Error: alloc shared mem from model_shm failed\n");
		return -1;
	}

	/* Some inputs are stored in segmented buffers, combine them */
	for (uint32_t i = 0; i < data->num_input_seg; i++) {
		if (data->input_seg[i].size <= inputs[input_index].size) {
			if (index_new) {
				inputs[input_index].addr = (char *)buf_addr + size_combined;
				size_combined = 0;
			}

			/* inputs[i].size is in bytes, so need to convert
			 * inputs[i].addr pointing to char first */
			memcpy((char *)(inputs[input_index].addr) + size_combined,
			       data->input_seg[i].addr, data->input_seg[i].size);
			size_combined += data->input_seg[i].size;

			if (size_combined == inputs[input_index].size) {
				((char *)inputs[input_index].addr)[size_combined] = 0;
				input_index++;
				index_new = true;
			} else {
				index_new = false;
...
static int print_string_tensor_content(OrtValue *input_tensor, uint32_t num_str)
{
	OrtStatus *status;
	size_t offsets[num_str];
	size_t len, elem_len;
	const OrtApi *ort_api = _odp_ml_glb->ort_api;

	_ODP_PRINT("Element contents: ");
	for (uint32_t i = 0; i < num_str; i++) {
		status = ort_api->GetStringTensorElementLength(input_tensor, i, &elem_len);
		if (check_ortstatus(status)) {
			_ODP_ERR("GetStringTensorElementLength() failed\n");
			return -1;
		}

		/* Element contents */
		char elem_content[elem_len + 1];

		memset(elem_content, 0, sizeof(elem_content));

		status = ort_api->GetStringTensorElement(input_tensor,
							 elem_len,
							 i,
							 elem_content);
		if (check_ortstatus(status)) {
			_ODP_ERR("GetStringTensorElement() failed\n");
			return -1;
		}
		_ODP_PRINT("%s", elem_content);
	}
	_ODP_PRINT("\n");

	status = ort_api->GetStringTensorDataLength(input_tensor, &len);
	if (check_ortstatus(status)) {
		_ODP_ERR("GetStringTensorDataLength() failed\n");
		return -1;
	}
	_ODP_PRINT("String data len: %zu\n", len);
	char str_content[len + 1];

	memset(str_content, 0, sizeof(str_content));
	status = ort_api->GetStringTensorContent(input_tensor,
						 (void *)str_content,
						 len, offsets, num_str);
	if (check_ortstatus(status)) {
		_ODP_ERR("GetStringTensorContent() failed\n");
		return -1;
	}

	_ODP_PRINT("String content: %s\n\n", str_content);
	return 0;
}

static int input_data_to_string_tensor(const odp_ml_input_info_t *input_info,
				       const ml_input_t *input_data, uint32_t batch_size,
				       OrtValue **input_tensor)
{
	...
	if (check_ortstatus(status) || !input_tensor[0]) {
		_ODP_ERR("CreateTensorAsOrtValue() failed\n\n");
		return -1;
	}

	/* Number of strings */
	num_str = input_data->size / sizeof(char *);
	status = ort_api->FillStringTensor(input_tensor[0],
					   (const char * const *)input_data->addr,
					   num_str);
	if (check_ortstatus(status)) {
		_ODP_ERR("FillStringTensor() failed\n\n");
		return -1;
	}
	...
	if (print_string_tensor_content(input_tensor[0], num_str)) {
		_ODP_ERR("print_string_tensor_content() failed\n");
		return -1;
	}
...
static int input_data_to_tensor(const odp_ml_input_info_t *input_info, const ml_input_t *input_data,
				uint32_t batch_size, OrtValue **input_tensor)
{
	...
	status = ort_api->CreateTensorWithDataAsOrtValue(_odp_ml_glb->memory_info,
							 input_data->addr,
							 input_data->size,
							 shape,
							 input_info->shape.num_dim,
...
{
	odp_ml_run_result_t result_local;
	ml_input_t input_combined[CONFIG_ML_MAX_INPUTS];
	...
	if (!(_odp_ml_glb->ml_config.run_mode_mask & ODP_ML_COMPL_MODE_SYNC)) {
		_ODP_ERR("Synchronous running is not enabled\n");
		return -1;
	}
	...
	fill_input_sizes(mdl, input_combined, param);

	if (fill_input_addrs(data, input_combined, ml_info->num_inputs, mdl)) {
		_ODP_ERR("Failed to fill input addrs\n");
		result_local.error_code = ML_BAD_INPUT;
		goto stash_back;
	}
	...
	for (uint32_t i = 0; i < ml_info->num_inputs; i++) {
		if (input_info[i].data_type == ODP_ML_DATA_TYPE_STR) {
			/* String uses a different calling pattern than other
			 * primitive data types */
			ret = input_data_to_string_tensor(&input_info[i],
							  &input_combined[i],
							  batch_size,
							  &input_tensor[i]);
		} else {
...
stash_back:
	/* Input data is segmented, put the shm back to stash */
	if (data->num_input_seg > ml_info->num_inputs && input_combined[0].addr) {
		uintptr_t addr_uintptr = (uintptr_t)input_combined[0].addr;

		ret = odp_stash_put_ptr(mdl->model_stash, &addr_uintptr, 1);
		if (ret != 1)
			_ODP_ERR("ML model stash put failed\n");
	}
...
static int read_config_file(ort_run_opts_t *opts)
{
	...
	if (opt_level_from_str(opt_level_str, &opts->graph_opt_level)) {
		_ODP_ERR("Graph otimize level %s not supported\n", opt_level_str);
		return -1;
	}
...
int _odp_ml_init_global(void)
{
	...
	status = ort_api->CreateCpuMemoryInfo(OrtArenaAllocator,
					      OrtMemTypeDefault,
					      &memory_info);
	if (check_ortstatus(status) || !memory_info) {
		_ODP_ERR("CreateCpuMemoryInfo failed\n");
		return -1;
	}
	_odp_ml_glb->memory_info = memory_info;
...
int _odp_ml_term_global(void)
{
	...
	if (_odp_ml_glb->memory_info)
		_odp_ml_glb->ort_api->ReleaseMemoryInfo(_odp_ml_glb->memory_info);
